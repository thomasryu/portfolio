...
export const Button = ({ color = 'white', ...props }: ButtonProps) => {
  ...
  return (
    <button
      className="group flex justify-center items-center w-10 h-10 lg:w-12 lg:h-12 rounded-full"
      onClick={() => props.router.push(props.href)}
    >
      <div
        className={`w-8 h-8 lg:w-10 lg:h-10 border-2 p-1.5 lg:p-2 rounded-full group-hover:scale-110 transition-transform duration-500 ${borderMap[color]}`}
      >
        <div
